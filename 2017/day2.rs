...
404	1757	358	2293	2626	87	613	95	1658	147	75	930	2394	2349	86	385";
    ...
    aoc_day2_part1(input);
    aoc_day2_part2(input);
}

fn aoc_day2_part1 (input: &str) {

    let rows = input.split("\n").map(|row|
        row.split("\t").fold(0, |acc, x|
            cmp::max(acc, x.parse::<i32>().unwrap())
        ) -
        row.split("\t").fold(9999999, |acc, x|
            cmp::min(acc, x.parse::<i32>().unwrap())
        )
    );

    let sum: i32 = rows.sum();

    println!("{}", sum);

}

fn aoc_day2_part2 (input: &str) {

    let mut vec = Vec::new();

    fn get_nth(nums: &str, n: usize) -> f64 {
        nums.split("\t")
            .map(|num| num.parse::<f64>().unwrap())
            .nth(n)
            .expect("wrong index or something?")
    }

    for row in input.split("\n") {
        let length = row.split("\t").fold(0, |acc, _| acc + 1);
        println!("{}", length);

        'outer: for i in 0..length {
            for j in 0..length {
                let first = get_nth(row, i);
                let second = get_nth(row, j);
                if first % second == 0.0 && i != j {
