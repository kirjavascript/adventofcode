...
fn main() {
    ...
    for capt in cmds {
        ...
        let reg1 = capt.get(1).unwrap().as_str();
        let op = capt.get(2).unwrap().as_str();
        let amt1 = capt.get(3).unwrap().as_str().parse::<i32>().unwrap();

        let reg2 = capt.get(4).unwrap().as_str();
        let cmp = capt.get(5).unwrap().as_str();
        let amt2 = capt.get(6).unwrap().as_str().parse::<i32>().unwrap();

        let cond: bool = match cmp {
            "!=" => *registers.entry(reg2).or_insert(0) != amt2,
            "<=" => *registers.entry(reg2).or_insert(0) <= amt2,
            "<" => *registers.entry(reg2).or_insert(0) < amt2,
            ">" => *registers.entry(reg2).or_insert(0) > amt2,
            "==" => *registers.entry(reg2).or_insert(0) == amt2,
        ...
        if cond {
            let reg1val = registers.entry(reg1).or_insert(0);
            if op == "inc" {
                *reg1val += amt1;
            }
            else {
...
const INPUT: &str = "ba dec 37 if znx != 0
