...
//                 U:_=>y+=1,
//             })[d[0]]()
//             positions.add([]+[Math.abs(x) + Math.abs(y), x, y])
//         }
//     })
//     return positions;
// })
// list = Array.from(a).filter(d => b.has(d)).map(d => d.split`,`[0]).sort((a,b)=>a-b)
...
//             r=[]+[x, y]
//             A=Math.abs;
//             a[r]=a[r]?o.push(A(x) + A(y)):1;
//         }
//     })
// })
// q = o.sort((a,b)=>a-b).shift()

// console.log(q)

// a={},o=[],store.get``.split`\n`.map(s=>{x=y=0,s.split`,`.map(s=>{for(i=+s.slice(1);i--;)({R:a=>x+=1,D:a=>y-=1,L:a=>x-=1,U:a=>y+=1})[s[0]](),r=[]+[x,y],A=Math.abs,a[r]=a[r]?o.push(A(x)+A(y)):1})}),q=o.sort((a,i)=>a-i).shift()
