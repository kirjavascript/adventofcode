...
I=$('*').innerText,A=0,I.replace(/\d+/g,(n,e)=>{for(i=a=0;3>i;a+=I.substring(e-=[1,-141,282][i++],e+([]+n).length+2));/[^\d\.\n]/.test(a)&&(A+=+n)}),A

// d3 p2

A=99;I=$('*').innerText.replace(/\*/g,_=>String.fromCharCode(A++));A=0;G={};I.replace(/\d+/g,(n,e)=>{for(i=a=0;3>i;a+=I.substring(e-=[1,-141,282][i++],e+([]+n).length+2));(V=[...a].find(c=>c>'b'))&&(G[V]?A+=G[V]*n:G[V]=n)}),A

// d4 p1

A=0;$('*').innerText.replace(/((\d+ +)+)\| +((\d+\s+)+)/g,(...[,a,,b])=>{q=s=>s.trim().split(/ +/);w=q(b).reduce((_, b)=>q(a).includes(b)?_+1:_,-1);~w?A+=2**w:0}),A

// d4 p2

c=0,(R=i=>{i.map(([i,l,n])=>{c++,R(A.slice(n,n+l.filter?.(l=>i.includes(l)).length))})})(A=$('*').innerText.split`\n`.map((i,l)=>[...i.slice(10).split(/ \| +/).map(i=>i.split(/ +/)),l+1])),--c
...
for(I=$('*').innerText.match(/\d+/g),A=1,q=4;q--;){for(n=+I[q],D=+I[q+4],v=0,i=n;i--;(n-(i+1))*(i+1)>D?v++:0);A*=v}A
